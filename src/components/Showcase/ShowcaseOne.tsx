...
function Model({ open, hinge, ...props }: {
    ...
    // Load model
    const { nodes, materials } = useGLTF('/mac-draco-transformed.glb') as MacGLTFResult
    ...
    return (
        <group
            ...
            dispose={null}
            scale={viewport.width > 70 && open ? viewport.width / 40 : viewport.width > 70 && !open ? viewport.width / 60 : 2.3}>
            <three.group rotation-x={hinge} position={[0, -0.04, 0.41]}>
                ...
                <Center
                    ...
                    <Text3D
                        scale={open ? 1 : 0}
                        material={visitHovered ? new THREE.MeshToonMaterial({ color: "blue" }) : new THREE.MeshToonMaterial({ color: "#F5E7DD" })}
                        font="/Kumar_One.json">
...
function Spinner() {
    return <Html center className="h-screen w-screen flex items-center justify-center">
        <svg className="animate-spin -ml-1 mr-3 h-5 sm:h-10 w-5 sm:w-10 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
